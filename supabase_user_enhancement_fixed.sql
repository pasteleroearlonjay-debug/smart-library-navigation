-- Smart Library System - User Enhancement for Supabase (FIXED VERSION)
...
-- View for user dashboard data (FIXED - checking if borrowing_records exists first)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'borrowing_records') THEN
        -- Create view with borrowing_records
        EXECUTE '
    ...
    ELSE
        -- Create view without borrowing_records
        EXECUTE '
        CREATE OR REPLACE VIEW user_dashboard_data AS
        SELECT 
            lm.id,
            ...
            lm.status,
            lm.last_login,
            0 as current_borrowed,
            0 as current_overdue,
            0 as upcoming_due,
            0 as overdue_count_new,
            COUNT(CASE WHEN brn.status = ''ready'' THEN 1 END) as ready_requests
        FROM library_members lm
        LEFT JOIN book_requests brn ON lm.id = brn.member_id
        GROUP BY lm.id, lm.name, lm.email, lm.membership_id, lm.profile_picture_url, lm.email_verified, 
...
-- Function to create deadline reminder notifications (only if borrowing_records exists)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'borrowing_records') THEN
        EXECUTE '
    ...
    ELSE
        ...
        RETURNS void AS $func$
        BEGIN
            -- Function placeholder - borrowing_records table not found
            RAISE NOTICE ''borrowing_records table not found - skipping deadline reminders'';
        END;
        $func$ LANGUAGE plpgsql';
    END IF;
END $$;
...
-- FIXES APPLIED:
-- - Added conditional logic to handle missing borrowing_records table
-- - Used ON CONFLICT DO NOTHING for sample data inserts
