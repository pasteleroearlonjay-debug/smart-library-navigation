-- Smart Library System - Library Members Setup for Supabase
...
-- First, let's add the missing columns to the existing library_members table
-- (Only run this if the table already exists but is missing columns)
...
CREATE TABLE IF NOT EXISTS borrowing_records (
    ...
    book_id BIGINT, -- This can reference your books table if you have one
    book_title VARCHAR(255), -- Store book info directly for simplicity
...
-- Based on the data shown in your interface
-- ========================================

-- Insert library members data
-- Using INSERT with ON CONFLICT to handle existing data
...
-- CREATE POLICY "Allow public read access" ON library_members FOR SELECT USING (true);
-- CREATE POLICY "Allow public read access" ON borrowing_records FOR SELECT USING (true);
...
-- Check if members were inserted
-- SELECT COUNT(*) as total_members FROM library_members;
...
-- Test a specific member's borrowing history
-- SELECT lm.name, br.book_title, br.borrowed_date, br.due_date, br.status 
-- FROM library_members lm 
-- JOIN borrowing_records br ON lm.id = br.member_id 
-- WHERE lm.name = 'John Doe';
